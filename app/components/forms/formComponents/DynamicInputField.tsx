    ...
    return (
      <div className="w-full">
        <label htmlFor={inputKey} className="inline-flex items-center gap-3 text-sm text-admin-light font-medium cursor-pointer select-none">
          ...
          />
          <span
            className={`relative inline-flex h-6 w-11 items-center rounded-full border transition-colors duration-200 peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-emerald-300/40 ${checked ? "bg-emerald-400/70 border-emerald-300/70" : "bg-white/15 border-white/20"} ${disabled ? "opacity-60" : ""}`}
            aria-hidden="true"
          >
            <span className={`inline-block h-4.5 w-4.5 rounded-full bg-white shadow transition-transform duration-200 ${checked ? "translate-x-5" : "translate-x-1"}`} />
          </span>
